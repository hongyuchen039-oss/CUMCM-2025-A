...
- `tests/test_q1_cylinder.py` — 单元测试 (73 测, 12 组 A-L, 全过)
...
- `RESULTS.md` — 增加完整圆柱冻结结果 + 收敛数据 + 本轮 FIX 后实测数字
...
### 方案 B: 完整圆柱严格遮蔽 (本轮冻结, FIX 后重跑)
- **有效遮蔽总时长 (fine 采样): 1.392384 s**
- 遮蔽区间: **(8.055704, 9.448088) s**
- 最大覆盖率 ρ_max = 1.000 (在 ρ=1 平台内, DIAG_STEP 网格最早达到 t = 8.100 s)
- ρ=1 平台 (0.01 s 网格): **(8.06, 9.44) s**, 总持续 **1.380 s**
...
- [x] 73 个单元测试全过 (A 10 + B 10 + C 5 + D 6 + E 6 + F 8 + G 5 + H 8 + I 5 + J 4 + K 3 + L 4)
...
- 是否接受 margin_max = 5.282478 m (0.001 s 局部加密) 报告口径?
