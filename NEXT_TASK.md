# 当前唯一任务

## 本轮任务
**TASK_002 Q1 点目标最小可运行基线 (BASELINE / EXPERIMENTAL)** —
审核返工已完成,等待外部审核。

### 范围
- 实现 Q1 运动学 + 点目标遮蔽判定 (方案 A 基线)
- 验证手算量: 投放点 R = (17620, 0, 1800) (由 Q1 给定输入推算)
- 验证起爆点 D = (17188, 0, 1736.496) (在 g=9.8 与共速假设下推算, 误差 ≤ 1e-6 m)
- 6 档扫描步长 (3 整倍数 + 3 非整倍数) 收敛
- 生成 SVG 图 (x-z 投影)

### 仅允许新建
1. `src/q1_baseline.py` (主程序)
2. `tests/test_q1_baseline.py` (单元测试 A-J 节, 42 测)
3. `outputs/q1/q1_baseline_plot.svg` (图像产物)

### 库限制
- 只使用 Python 标准库
- 不得强制安装 numpy / scipy / pandas / matplotlib

### 本轮明确不做
- 不生成 result1/2/3.xlsx (TASK_003 之前)
- 不实现方案 B 完整圆柱正式模型 (Q2 之前冻结)
- 不实现 Q2 最优化
- 不修改官方原模板 / problem/*.pdf / 题目及模板/ / desktop.ini
- 不自动 merge 任何 PR
- 不动 main

### 本轮完成标准
1. 投放点 R、起爆点 D 与代码输出一致 (误差 ≤ 1e-6 m) ✓
2. M1 速度方向正确 (|v| = 300, 不是 (-300, 0, 0)) ✓
3. 42 个单元测试全过 ✓
4. 6 档扫描步长结果稳定 (max |Δt| = 4.12e-9 s) ✓
5. 区间端点 |f(b)| ≤ 5e-6 ✓
6. SVG 合法可解析, 含关键点标签 ✓
7. commit + push 到 task/TASK_002-q1-baseline ✓ (在审核返工提交 + push)
8. 更新同一 Draft PR #2 描述 ✓

### 计算结果 (来自 src/q1_baseline.py 当前输出)
- 投放点: (17620.000000, 0, 1800.000000) m
- 起爆点: (17188.000000, 0, 1736.496000) m (D.z = 1800 − 0.5·9.8·3.6²)
- M1 速度: (-298.511157, 0, -29.851116) m/s, |v| = 300 m/s
- M1 到达假目标理论时刻: 66.999 s
- 遮蔽区间 (方案 A): (8.013006, 9.448088) s
- **有效遮蔽总时长 (BASELINE / EXPERIMENTAL): 1.435082 s**
- 等级: 仅 BASELINE / EXPERIMENTAL, 不得冒充 VERIFIED / FINAL

## 本轮后续 (待审核后决定)

PR #2 合并后进入 TASK_003: 完整圆柱遮蔽判定冻结与 Q1 对照。

TASK_003 任务目的:
- 冻结完整圆柱可见轮廓/表面判定 (方案 B, FACTS.md §15)
- 确定采样或解析方法
- 确定严格遮蔽与覆盖比例指标
- 做采样收敛验证
- 用 Q1 固定参数比较点目标与完整圆柱结果

不得直接进入 Q2 单弹优化 (必须先冻结完整圆柱模型)。
