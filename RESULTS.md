...
### 方案 B — 完整圆柱严格遮蔽 (FULL-CYLINDER CANDIDATE / EXPERIMENTAL, 本轮 FIX 后重跑)
- 有效遮蔽总时长 (fine 采样): **1.392384 s**
- 遮蔽区间 (fine): **(8.055704, 9.448088) s**
- 最大覆盖率 ρ_max = **1.000** (在 ρ=1 平台内; 诊断网格最早达到时刻 t = **8.100 s**)
- ρ=1 平台区间 (DIAG_STEP=0.01 s 网格): **(8.06, 9.44) s**, 总持续 **1.380 s**
- 最大严格裕量 margin_max (0.001 s 局部加密扫描): **5.282478 m** @ t = **9.418317 s**
  (相对于 SVG 0.05 s 网格上观察到的 t=9.400 s, ±0.05 s 窗口)
...
| H | 8 | 空间收敛 (check_passed_true/区间数一致/起点差/终点差/总时长差/coverage 差/margin 差/残差) |
| I | 5 | 时间收敛 (check_passed_true/区间数一致/起终点总时长/残差/多区间合成) |
| J | 4 | SVG 解析 (合法 XML/有标题/有等级标签/有时间面板块) |
| K | 3 | margin 局部加密 + coverage 平台 + 时间序列有限性 |
| L | 4 | 几何 API 输入校验 (非有限 m/c/radius 抛异常 + 轨迹回调注入) |

`python -m unittest tests.test_q1_baseline -v` → 42 / 42 ok.
`python -m unittest tests.test_q1_cylinder -v` → 73 / 73 ok.
...
- 本轮 42 (方案 A) + 73 (方案 B) 单元测试 + 6 档扫描 + 三档空间采样 + 三档时间采样
  + coverage_plateau + margin 局部加密共同验证, 未伪造任何"论文对比"或"权威背书".
